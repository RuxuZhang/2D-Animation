void keyPressed()  // executed each time a key is pressed: sets the Boolean "keyPressed" till it is released   
    ...
    if(key=='1') method=1;                  
    if(key=='2') method=2;
    if(key=='3') ;// pointsCount=4;//add a key to turn back//*******by Ruxu Zhang*******
    ...
    if(key=='9') ;
    if(key=='0') method=0;
    if(key=='-') if (numGrid > 1) numGrid--; // use control number of grids //{A=Point[0]; B=Point[1]; C=Point[2]; D=Point[3];} Delete previous functionality
    if(key=='=') if (numGrid < 800) numGrid++;  // use control number of grids

    if(key=='a') animating=!animating;
    if(key=='b') ; 
    if(key=='c') showCoon = (showCoon+1) % 3; 
    if(key=='d') ;  
    if(key=='e') ;
    if(key=='f') fill=!fill;
    if(key=='g') showRegister = !showRegister; 
    if(key=='h') ;
    if(key=='i') ;
    if(key=='j') ;
    if(key=='k') ; 
    ...
    if(key=='n') nevSmooth = !nevSmooth;
    if(key=='o') ;  
    if(key=='p') ;
    if(key=='q') ; 
    if(key=='r') {readPoints("data/points.pts"); } 
    if(key=='s') showLPM=!showLPM;
    if(key=='t') texturing = (texturing+1) % 3; 
    if(key=='u') ;
